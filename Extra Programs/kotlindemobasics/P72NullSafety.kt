...
where as !!(not null assertion operator) works at run time.
Kotlin’s type system is aimed to eliminate the jeopardy of
 null reference from the code because it is a billion
dollar mistake. NullPointerExceptions are thrown by the
...
that has concept of null reference then he must has
...
4.Java interoperations such as attempts to access a member
on a null reference , generics type with incorrect
nullability.


-->Nullable and Non-Nullable Types in Kotlin –
Kotlin type system has distinguish two types of references
